...
function getDeck(data, callback) {
	request
		.get('http://localhost:3002/roll/' + data.id)
...
app.get('/roller/:id', function (req, res) {
	...
	request
		.get('http://localhost:3002/roller/' + id)
		.set('Accept', 'application/json')
...
app.post('/roller', function (req, res) {
	...
	if (!dice || !Array.isArray(dice)) {
		res.json(200, { error: 'dice must be specified as an array i.e. [6, 6]' });
		return;
	}
	aRoller = roller.createRoller(dice, bonus);
	request
		.post('http://localhost:3002/roller')
		.send({ dice: JSON.stringify(aRoller.getDice()),
			bonus: JSON.stringify(aRoller.getBonus()) })
		.set('Content-type', 'application/json')
		.set('Accept', 'application/json')
		.end(function (err, response) {
			if (err) {
...
app.put('/roller', function (req, res) {
	...
	if (update !== 'roll' && update !== 'add' && update !== 'remove') {
